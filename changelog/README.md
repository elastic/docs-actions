...
Products that publish release notes keyed by **date** rather than a semver tag — Cloud Serverless, Cloud Hosted — cut their bundle at promotion time from a **git commit range**. The promotion system hands off two commit hashes from a protected branch: the previously published endpoint ref and the newly published one. Nothing else is staged externally: `docs-builder` derives the PR list from the range itself (GitHub compare API + GraphQL `associatedPullRequests`) and records the end ref in the bundle as `git_ref` metadata.

By default, bundling sources entries from the public CDN only. PRs that don't have a matching changelog are warned and omitted from the bundle. If you want to turn on automatic creation of transitory changelog entries when there's no matching changelog, set `infer: true` in the action input or `bundle.profiles.<name>.infer_missing_changelogs: true` in your `docs/changelog.yml`. Note that these generated changelogs are not stored and therefore cannot be re-used and are difficult to update.
...
- By default, bundling sources entries from the **CDN/S3 pool only**: each PR in the commit range must have a published changelog entry in S3; PRs without one are warned and omitted. To enable synthesis from PR metadata, set `infer: true` in the action input or add `infer_missing_changelogs: true` to your profile in `docs/changelog.yml`.
