...
| `output`               | Output file path for the bundle, relative to the repo root. Optional; when omitted in profile mode, the plan resolves `{repo}-{product}-{version}.yaml` under the profile's output_directory if set, otherwise bundle.output_directory.<br> | `false`  | ` `                   |
| `repo`                 | GitHub repository name. Used in the profile-mode bundle filename and CDN URL. Falls back to bundle.repo in changelog.yml, then git remote origin.<br>                                                                                       | `false`  | ` `                   |
