...
steps:
  ...
  - name: Run codespell
    ...
    run: |
      ...
      # target-files overrides target-path and scope-mode: scan exactly the
      # listed files (newline- or comma-separated, docs-root-relative).
      if [ -n "$TARGET_FILES" ]; then
        SELECTION_MODE="files"
        REQUESTED_SCOPE_MODE="files"
      ...
      EOF
        echo "Codespell scan (file list): requested=$REQUESTED_COUNT in_scope=$IN_SCOPE_COUNT finding_count=$FINDING_COUNT exit_code=$RC"
        exit 0
      fi
...
- Files mode (`selection_mode` is `files`): `file list — <count> typos across <files> files`.

**Files mode**: when `selection_mode` is `files`, the caller supplied an explicit file list via `target-files`; scan exactly those files. Use the scope-summary line `Scanned <in_scope_count> of <requested_count> requested files; codespell raised <finding_count> raw findings, <count> retained after filtering.`
